...
Все секреты и настройки — в `.env` (см. `.env` в корне; в Git не попадает). Минимум для запуска:
...
Опционально (есть в `.env` закомментированными): переключение на OpenRouter, имена моделей по ролям, креды KB-репо, `ADMIN_USER_ID` (первый админ), путь к БД.
